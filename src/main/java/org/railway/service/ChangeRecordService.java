...
public class ChangeRecordService {
     ...
     */
    @Transactional
    public ChangeRecordResponse createChangeRecord(ChangeRecordRequest request) {
        ...
        changeRecord.setOriginalTicket(originalTicket);
        changeRecord.setNewTicket(newTicket);
     ...
     */
    @Transactional
    public ChangeRecordResponse updateChangeRecordStatus(Long id, Integer status) {
        ChangeRecord changeRecord = changeRecordRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("改签记录不存在"));
